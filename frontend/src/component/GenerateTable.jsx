...
export default function GenerateTable() {
  const [rows, setRows] = useState(
    INITIAL_ROWS.map((r) => ({ day: r.day, morning: 0, evening: 0, total: 0, min: r.min, max: r.max }))
  );
  ...
  const handleGenerate = () => {
    ...
    setRows(final.map((r) => ({ day: r.day, morning: r.morning, evening: r.evening, total: r.total, min: r.min, max: r.max })));
  ...
  const handleExport = () => {
    ...
    data.push({ Day: "TOTAL", Morning: grandM, Evening: grandE, Total: grandT });

    const ws = XLSX.utils.json_to_sheet(data, { header: ["Day", "Morning", "Evening", "Total"] });
    ...
    <div className="w-full min-h-screen bg-gray-50 py-8">
      <div className="w-full max-w-[1400px] mx-auto px-4">
        <header className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-3xl font-extrabold text-gray-900">Generate Table</h1>
          </div>

          <div className="flex items-center gap-3">
            <button
              onClick={handleGenerate}
              className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-green-600 to-green-500 text-white rounded-lg shadow hover:from-green-700 transition"
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2 border rounded-lg bg-white hover:bg-gray-50"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor" aria-hidden>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Main table card */}
          <div className="col-span-1 lg:col-span-2 bg-white rounded-xl shadow p-4 overflow-x-auto">
            ...
            <table className="min-w-full w-full table-auto divide-y divide-gray-200">
              <thead className="bg-gray-100 sticky top-0">
                <tr>
                  ...
                  <th className="px-4 py-3 text-right text-sm font-medium text-gray-700">Morning</th>
                  <th className="px-4 py-3 text-right text-sm font-medium text-gray-700">Evening</th>
              ...
              <tbody className="bg-white divide-y divide-gray-100">
                {rows.map((r, idx) => (
                  <tr key={r.day} className={idx % 2 ? "bg-white" : "bg-gray-50 hover:bg-gray-100"}>
                    <td className="px-4 py-3 text-sm text-gray-800">{r.day}</td>
                    <td className="px-4 py-3 text-sm text-right font-mono">{(r.morning || 0).toFixed(3)}</td>
                    <td className="px-4 py-3 text-sm text-right font-mono">{(r.evening || 0).toFixed(3)}</td>
                    <td className="px-4 py-3 text-sm text-right font-semibold font-mono">{(r.total || 0).toFixed(3)}</td>
                    <td className="px-4 py-3 text-sm text-gray-500">{r.min.toFixed(3)} — {r.max.toFixed(3)}</td>
                  </tr>
                ))}

                <tr className="bg-gray-100 font-semibold">
                  <td className="px-4 py-3">TOTAL</td>
                  <td className="px-4 py-3 text-right font-mono">{grandMorning.toFixed(3)}</td>
                  <td className="px-4 py-3 text-right font-mono">{grandEvening.toFixed(3)}</td>
                  <td className="px-4 py-3 text-right font-mono">{grandTotal.toFixed(3)}</td>
          ...
          <aside className="bg-white rounded-xl shadow p-4 flex flex-col gap-4">
            <div>
              ...
              <div className="text-2xl font-bold text-gray-900">{grandTotal.toFixed(3)}</div>
              <div className="text-xs text-gray-500 mt-1">Morning: {grandMorning.toFixed(3)} • Evening: {grandEvening.toFixed(3)}</div>
            </div>

            <div className="pt-2 border-t">
              <h4 className="text-sm text-gray-600 mb-2">Quick actions</h4>
              <button onClick={() => { setRows(INITIAL_ROWS.map((r) => ({ day: r.day, morning: 0, evening: 0, total: 0, min: r.min, max: r.max }))); setMessage('Reset to initial values'); }} className="w-full px-3 py-2 rounded-md bg-blue-500 text-white hover:bg-blue-600 transition">Reset</button>
            </div>

            <div className="pt-2 border-t">
              <h4 className="text-sm text-gray-600 mb-2">Notes</h4>
              <p className="text-xs text-gray-500">Values are generated randomly within each row range then scaled to sum to 100. Small rounding adjustments are made.</p>
            </div>
          </aside>
        </div>
      </div>
